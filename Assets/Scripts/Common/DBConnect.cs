...
public static class DBConnect
{
    ...
    public static async UniTask<(bool success, string error)> VerifyLoginAsync(string login, string password, Action endAction)
    {
        ...
        using (var connection = new MySqlConnection(connectionString))
        {
            ...
            try
            {
                SaveService.SaveLocalUser(10, password);
    ...
    public static async UniTask<bool> SaveLevelDataAsync(string level, int userId, float completionTime)
    {
        await UniTask.SwitchToThreadPool();
        ...
        using (var connection = new MySqlConnection(connectionString))
        {
            ...
            try
            {
                await connection.OpenAsync();
                command.Connection = connection;
                command.CommandText = $"INSERT INTO {level} (user_id, completion_time) VALUES (@user_id, @completion_time) ON DUPLICATE KEY UPDATE completion_time = @completion_time";
    ...
    public static async UniTask<(bool success, float completionTime)> LoadLevelDataAsync(string level, int userId)
    {
        await UniTask.SwitchToThreadPool();
        ...
        using (var connection = new MySqlConnection(connectionString))
        {
            ...
            try
            {
                ...
                command.Connection = connection;
                command.CommandText = $"SELECT completion_time FROM {level} WHERE user_id = @user_id";
                command.Parameters.AddWithValue("@user_id", userId);
