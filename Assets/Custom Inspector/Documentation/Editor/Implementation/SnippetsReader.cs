...
namespace CustomInspector.Documentation
{
    ...
    public class SnippetsReader : ScriptableObject
    {
        ...
        public void DrawDescription(Rect position, string text)
        {
        ...
        public void DrawInvalidOnScriptables(Rect position)
        {
            DrawProperties.DrawBorder(position, false);
            position = GuidanceWindow.Shrinked(position, padding);

            EditorGUI.HelpBox(position, "This attribute cannot be used on ScriptableObjects." +
                                        "\nTry the code on a monobehaviour.",
                                        MessageType.Warning);
        }
